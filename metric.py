...
def ndcg_at_k(obj: Data, score, eval=True):
    """
    Calculate the DCG score
    :param obj: A instance of Data.
    :param score: The predicted CVR score for evaluation.
    ...
    if eval:
        for k in [2, 4, 6]:
            ...
            order = np.log2(np.arange(k) + 2)
            dcg = np.divide(convert_k, order)
            convert_k[:, ::-1].sort() # Descending order, axis = 1
            idcg = np.divide(convert_k, order)
            ndcg_k.append(np.sum(dcg) / np.sum(idcg))
    else:
        for k in [1, 2, 3, 4, 5, 6]:
            convert_k = convert[:, :k]
            order = np.log2(np.arange(k) + 2)
            dcg = np.divide(convert_k, order)
            convert_k[:, ::-1].sort()  # Descending order
            idcg = np.divide(convert_k, order)
            ndcg_k.append(np.sum(dcg) / np.sum(idcg))
    return np.array(ndcg_k)
...
if __name__ == '__main__':
    file = open("../data/real-world/coat_cur0.data", "rb")
    coat = pickle.load(file)
    file.close()
    list = dcg_at_k(coat, np.zeros([coat.user_num * coat.test_user_item]))
    print(list)
    list = ndcg_at_k(coat, np.zeros([coat.user_num * coat.test_user_item]))
    print(list)
